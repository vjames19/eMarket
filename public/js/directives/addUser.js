      ...
      return {
        ...
        controller: function ($scope, Restangular) {
          $scope.submit = function () {
            $.mobile.loading('show');
            Restangular.all('api/users').post($scope.addUser.user).then(function(user) {
              Restangular.one('api/users', user.userId).all('mailAddresses').post($scope.addUser.mailAddress);
              Restangular.one('api/users', user.userId).all('billAddresses').post($scope.addUser.billAddress);
              Restangular.one('api/users', user.userId).all('creditCards').post($scope.addUser.creditCard);
              $.mobile.loading('hide');
